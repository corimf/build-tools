...
var DisplayScriptInformation = function (scriptWorkflow) {
    ...
    rl.question("Hit ENTER to continue", function (enter) {
        ...
        rl.on('close', function () {
            console.log("\n");
            rl.close();
            //Exit the program through tests.reportStatus to clean up temp dir.
            tests.reportStatus(DONE);
        });
...
var RunPreliminaryTests = function (platformSpecificCode) {
    ...
    tests.reportStatus(!shelljs.test('-d', '.temp'));
    ...
    repos = settings.PLATFORM_REPO ==="cordova-blackberry" ? (settings.PLUGINS.concat(settings.PLATFORM_REPO)).concat("cordova-blackberry-plugins") : settings.PLUGINS.concat(settings.PLATFORM_REPO);
    ...
    if (!settings.PROJECT_ONLY) {
        ...
        for (var i = 0; i < repos.length; i++) {
            ...
            tests.reportStatus((execOutput = shelljs.exec(archiveCommand, {
                            silent : false
                        })).code == 0, execOutput.output);
            console.log("archived".green);
            shelljs.cd('..');
        }
    }

    console.log('Checking that the platform and plugins have the tag ' + (settings.NEW_TAG).green + '...');
    for (var i = 0; i < repos.length; i++) {
        console.log(repos[i] + ': ');
        shelljs.cd(repos[i]);
        tests.reportStatus((execOutput = shelljs.exec('git rev-parse ' + settings.NEW_TAG, {
                        silent : true
                    })).code == 0, execOutput.output);
    ...
    console.log('Checking that the platform and plugins are checked out at the tag ' + (settings.NEW_TAG).green +'...');
    ...
    if (settings.PROJECT_ONLY) {
        for (var i = 0; i < repos.length; i++) {
            ...
            tests.reportStatus((execOutput = shelljs.exec(archiveCommand)).code == 0, execOutput.output);
            console.log("checked".green);
            shelljs.cd('..');
        }
    }
    // If not building project only, make sure that the branch matches the latest tag in Git
        ...
        for (var i = 0; i < repos.length; i++) {
            console.log(repos[i] + ': ');
            shelljs.cd(repos[i]);
            var actual = shelljs.exec('git rev-parse ' + settings.BRANCH, {
                    silent : true
                });
            tests.reportStatus(actual.code == 0, actual.output);
            var desired = shelljs.exec('git rev-parse ' + settings.NEW_TAG, {
                    silent : true
                });
            tests.reportStatus(desired.code == 0, desired.output);
            tests.reportStatus(actual.output.trim() === desired.output.trim(), "Branch hash doesn't match latest tag hash");
            shelljs.cd('..');
        }
    }
    ...
    for (var i = 0; i < repos.length; i++) {
        ...
        shelljs.cd(repos[i]);
        if (unzip == true) {
            tests.reportStatus((execOutput = shelljs.exec('unzip ' + tmpDir + path.sep + repos[i] + '.zip', {
                            silent : true
                        })).code == 0, execOutput.output);
        } else {
            tests.reportStatus((execOutput = shelljs.exec('7z x ' + tmpDir + path.sep + repos[i] + '.zip', {
                            silent : true
                        })).code == 0, execOutput.output);
        }
        shelljs.cd('..');
    ...
    if (settings.PLUGINS.length > 0) {
        ...
        tests.reportStatus((execOutput = shelljs.exec('npm install', {
                        silent : true
                    })).code == 0, execOutput.output);
        shelljs.cd('..');
    }
...
var BuildProject = function (platformSpecificCode) {
    ...
    if (!settings.PROJECT_ONLY) {
        ...
        tests.reportStatus(!shelljs.test('-d', tmpDir + path.sep + SNAPSHOT_DIR));
    }

    var execPath = platform === "wp8" ? (Number(settings.BRANCH.substring(0, 3)) > 3.6 ? path.join(pathCorimf, 'cordova-' + platform, 'bin', 'create') : path.join(pathCorimf, 'cordova-' + platform, 'wp8', 'bin', 'create')) : (platform === "blackberry10" && Number(settings.BRANCH.substring(0, 3)) <= 3.6) ? path.join(pathCorimf, settings.PLATFORM_REPO, platform, 'bin', 'create') : path.join(pathCorimf, 'cordova-' + platform, 'bin', 'create');
    var cmd = execPath + ' ' + PROJECT_DIR + ' ' + PROJECT_NAME + ' ' + PROJECT_NAME;
    console.log('Creating a new ' + (platform.toUpperCase()).cyan + ' Project...');
    console.log('Running: \n' + cmd);
    tests.reportStatus((execOutput = shelljs.exec(cmd, {
            silent : true
        })).code == 0,execOutput.output);

    //Install all plugins
    //Special workaround for BB10, install BB plugins first
    var pluginsToInstall =  (platform ==="blackberry10" && settings.BRANCH === "3.1.0esr") ? ([path.join('cordova-blackberry-plugins','plugin','com.blackberry.utils'),path.join('cordova-blackberry-plugins','plugin','org.apache.cordova.blackberry10.pimlib')]).concat(settings.PLUGINS) : settings.PLUGINS;
    ...
    for (var i = 0; i < pluginsToInstall.length; i++) {
        ...
        var execCommand = 'node ' + pathPlugman + ' --debug install --platform ' + platform + ' --project ' + pathProject + ' --plugin ' + tmpDir + path.sep + pluginsToInstall[i];
        tests.reportStatus((execOutput = shelljs.exec(execCommand, {
                        silent : true
                    })).code == 0, execOutput.output);
    }
    ...
    if (settings.MOBILESPEC) {
        ...
        tests.reportStatus(!shelljs.test('-d', MOBILESPEC_DIR));
    ...
    if (!settings.PROJECT_ONLY) {
        ...
        if (unzip == true) {
            ...
            shelljs.cd(tmpDir);
            tests.reportStatus(shelljs.exec('zip -r ' + unzipFile + ' .' + path.sep + SNAPSHOT_DIR, {
                    silent : false
                }).code == 0);
            shelljs.cd(cwd);
        }
...
function PreliminaryTests() {
    ...
    if (shelljs.exec('unzip', {
            silent : true
        }).code == 0)
        unzip = true;
    else {
        console.log("Checking if 7zip is installed");
        tests.reportStatus((execOutput = shelljs.exec('7za', {
                        silent : true
                    })).code != 0, execOutput.output);
    }
    if (shelljs.exec('ulimit', {
            silent : true
        }).code == 0) {
        console.log("Checking 'ulimit' size for file descriptors");
        var ulimitSize = Number(shelljs.exec('ulimit -n').output);
        if (ulimitSize < 4192)
            tests.reportStatus(false, "'ulimit' size too small, please run command 'ulimit -n 4192' before running script");
    }
    ...
    if (majorBranchNum > 3.1 && settings.PROJECT_ONLY == false) {
        tests.reportStatus(false, "Cannot build full deliverable for branches above 3.1 using this script, please either set PROJECT_ONLY to true in settings or use other script");
    }
}
